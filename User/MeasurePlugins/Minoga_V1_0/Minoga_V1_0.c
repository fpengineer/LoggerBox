/*****************************************************************************************************
    ...
    Overview: Plugin Minoga - V1.0
    
*******************************************************************************************************/
#include <stdio.h>
#include <stdint.h>

#include "Minoga_V1_0\Minoga_V1_0.h"
#include "Minoga_V1_0\types.h"
#include "Minoga_V1_0\defs.h"
...
void Minoga_V1_0( PluginResult_t *pluginResult, PluginCommand_t pluginCommand, int32_t *tactLength_ms )
{
    ...
    switch ( pluginCommand )
    {
        case CMD_PLUGIN_RUN:
        {
            ...
            HwAPI_Terminal_SendMessage( "Minoga_V1_0 - Run\n" );
#endif
            /* Read plugin config file */
            HwAPI_Terminal_SendMessage( "Minoga_V1_0 - Start reading of measure plan file\n" );
        ...
        {
#ifdef PLUGINS_DEBUG_INFO
            HwAPI_Terminal_SendMessage( "Minoga_V1_0 - Stop\n" );
#endif
            /* Clear all sources */
            HwAPI_VoltageSource_ClearAll();
        ...
        {
#ifdef PLUGINS_DEBUG_INFO
            HwAPI_Terminal_SendMessage( "Minoga_V1_0 - Tact\n" );
#endif
            ...
            if ( cfgMeasureEnable.enableIC1_Freq_PWM )
            {
                ...
                measureValues.valueIC1_PWM.frequency_kHz *= FREQUENCY_DIVIDER;
                measureValues.valueIC1_PWM.pulseHigh_ns *= FREQUENCY_DIVIDER;
                measureValues.valueIC1_PWM.pulseLow_ns *= FREQUENCY_DIVIDER;
            }
        ...
        {
#ifdef PLUGINS_DEBUG_INFO
            HwAPI_Terminal_SendMessage( "Minoga_V1_0 - Error\n" );
#endif
        ...
        {
#ifdef PLUGINS_DEBUG_INFO
            HwAPI_Terminal_SendMessage( "Minoga_V1_0 - Idle\n" );
#endif
                                     ...
                                     CfgDatafileSettings_t *cfgDatafileSettings )
{
    ...
    snprintf( measurePlanFilename, GetSizeof_measurePlanFilename(), "MeasurePlan_"PLUGIN_14_NAME".ini" );
         ...
         FATFS_OK != HwAPI_FatFs_INI_GetKeyString( "DatafileSettings", "Delimiter", measurePlanFilename, cfgDatafileSettings->delimiter, MEASURE_PLAN_FILE ) )
    {
        ...
        HwAPI_Terminal_SendMessage( "Measure plan file of the plugin "PLUGIN_14_NAME" read - Error.\n" );
        pluginResult.error = 1;
        pluginResult.errorCode = PLG_ERR_READ_MEASURE_PLAN_FILE;
        snprintf( pluginResult.message, sizeof( pluginResult.message ), "Plugin "PLUGIN_14_NAME". Error comes during reading of measure plan file.\n" );
        return pluginResult;
    }
         
    /* Send readed values to terminal */     
    HwAPI_Terminal_SendMessage( "Measure plan file of the plugin "PLUGIN_14_NAME" read - Success.\n" );
...
static void CreateHeaderString( char *string, int32_t stringSize, CfgMeasureEnable_t *cfgMeasureEnable, char *delimiter )
{
    ...
    if ( cfgMeasureEnable->enableIC1_Freq_PWM )
    {
        ...
        snprintf( string, stringSize, "%sIC1_Q%s", string, delimiter );        
    }
            
    if ( cfgMeasureEnable->enableIC2_Freq_PWM )
    {
        snprintf( string, stringSize, "%sIC2_Freq, kHz%s", string, delimiter );        
        snprintf( string, stringSize, "%sIC2_Q%s", string, delimiter );        
    }
            
    if ( cfgMeasureEnable->enableIC3_Freq_PWM )
    {
        snprintf( string, stringSize, "%sIC3_Freq, kHz%s", string, delimiter );        
        snprintf( string, stringSize, "%sIC3_Q%s", string, delimiter );        
    }
            
    if ( cfgMeasureEnable->enableIC4_Freq_PWM )
    {
        snprintf( string, stringSize, "%sIC4_Freq, kHz%s", string, delimiter );        
        snprintf( string, stringSize, "%sIC4_Q%s", string, delimiter );        
    }
...
static void CreateMeasureString( char *string, int32_t stringSize, MeasureValues_t *measureValues, CfgMeasureEnable_t *cfgMeasureEnable, char *delimiter )
{
    ...
    if ( cfgMeasureEnable->enableIC1_Freq_PWM )
    {
        ...
        snprintf( string, stringSize,                                                                                                                   \
                  "%s%.1f%s",                                                                                                                           \
                  string,                                                                                                                               \
                  ( measureValues->valueIC1_PWM.pulseHigh_ns + measureValues->valueIC1_PWM.pulseLow_ns ) / measureValues->valueIC1_PWM.pulseHigh_ns,    \
                  delimiter );        
    }
    ...
    if ( cfgMeasureEnable->enableIC2_Freq_PWM )
    {
        ...
        snprintf( string, stringSize,                                                                                                                   \
                  "%s%.1f%s",                                                                                                                           \
                  string,                                                                                                                               \
                  ( measureValues->valueIC2_PWM.pulseHigh_ns + measureValues->valueIC2_PWM.pulseLow_ns ) / measureValues->valueIC2_PWM.pulseHigh_ns,    \
                  delimiter );        
    }
    ...
    if ( cfgMeasureEnable->enableIC3_Freq_PWM )
    {
        ...
        snprintf( string, stringSize,                                                                                                                   \
                  "%s%.1f%s",                                                                                                                           \
                  string,                                                                                                                               \
                  ( measureValues->valueIC3_PWM.pulseHigh_ns + measureValues->valueIC3_PWM.pulseLow_ns ) / measureValues->valueIC3_PWM.pulseHigh_ns,    \
                  delimiter );        
    }
    ...
    if ( cfgMeasureEnable->enableIC4_Freq_PWM )
    {
        ...
        snprintf( string, stringSize,                                                                                                                   \
                  "%s%.1f%s",                                                                                                                           \
                  string,                                                                                                                               \
                  ( measureValues->valueIC4_PWM.pulseHigh_ns + measureValues->valueIC4_PWM.pulseLow_ns ) / measureValues->valueIC4_PWM.pulseHigh_ns,    \
                  delimiter );        
    }
                                             ...
                                             CfgDatafileSettings_t *cfgDatafileSettings )
{
    ...
    snprintf( measureDataFilename, sizeFilename, "measure/"PLUGIN_14_NAME"/%s_%s.csv", cfgDatafileSettings->prefixDatafileName, pluginsTempString );
