/*****************************************************************************************************
    ...
    Overview: Plugin Egorov 1290EF1 - V1.0
...
void plg1290EF1_V1_0( PluginResult_t *pluginResult, PluginCommand_t pluginCommand, int32_t *tactLength_ms )
{
    ...
    switch ( pluginCommand )
    {
        case CMD_PLUGIN_RUN:
        {
            ...
            HwAPI_Terminal_SendMessage( "1290EF1_V1_0 - Run\n" );
#endif
            /* Read plugin config file */
            HwAPI_Terminal_SendMessage( "1290EF1_V1_0 - Start reading of measure plan file\n" );
            ...
            if ( !pluginResult->error )
            {
                ...
                if ( cfgDatafileSettings.enableDatafile )
                {
                    CreateMeasurementFile( measureDataFilename,
                                           ...
                                           &cfgDatafileSettings );
                    stringsCounter = 8;
                    HwAPI_Terminal_SendMessage( "1290EF1_V1_0 - measure data file created\n" );
                }
        ...
        {
#ifdef PLUGINS_DEBUG_INFO
            HwAPI_Terminal_SendMessage( "1290EF1_V1_0 - Stop\n" );
#endif
            /* Clear all sources */
            HwAPI_VoltageSource_ClearAll();
        ...
        {
#ifdef PLUGINS_DEBUG_INFO
            HwAPI_Terminal_SendMessage( "1290EF1_V1_0 - Tact\n" );
#endif
            ...
            {
                HwAPI_DAQ_ADC_SelectInput( ADC_CHANNEL_IC1, ADC_SOURCE_ICn_Comp );
                HwAPI_DAQ_ADC_GetAveraged( &measureValues.valueIC1_Comp, ADC_RANGE_ICn_Comp, ADC_N_AVERAGES );
            }
            
            if ( cfgMeasureEnable.enableIC2_Comp )
            {
                HwAPI_DAQ_ADC_SelectInput( ADC_CHANNEL_IC2, ADC_SOURCE_ICn_Comp );
                HwAPI_DAQ_ADC_GetAveraged( &measureValues.valueIC2_Comp, ADC_RANGE_ICn_Comp, ADC_N_AVERAGES );
            }
            
            if ( cfgMeasureEnable.enableIC3_Comp )
            {
                HwAPI_DAQ_ADC_SelectInput( ADC_CHANNEL_IC3, ADC_SOURCE_ICn_Comp );
                HwAPI_DAQ_ADC_GetAveraged( &measureValues.valueIC3_Comp, ADC_RANGE_ICn_Comp, ADC_N_AVERAGES );
            }
            
            if ( cfgMeasureEnable.enableIC4_Comp )
            {
                HwAPI_DAQ_ADC_SelectInput( ADC_CHANNEL_IC4, ADC_SOURCE_ICn_Comp );
                HwAPI_DAQ_ADC_GetAveraged( &measureValues.valueIC4_Comp, ADC_RANGE_ICn_Comp, ADC_N_AVERAGES );
            }
        ...
        {
#ifdef PLUGINS_DEBUG_INFO
            HwAPI_Terminal_SendMessage( "1290EF1_V1_0 - Error\n" );
#endif
        ...
        {
#ifdef PLUGINS_DEBUG_INFO
            HwAPI_Terminal_SendMessage( "1290EF1_V1_0 - Idle\n" );
#endif
                                     ...
                                     CfgDatafileSettings_t *cfgDatafileSettings )
{
    ...
    snprintf( measurePlanFilename, GetSizeof_measurePlanFilename(), "MeasurePlan_"PLUGIN_12_NAME".ini" );
    ...
    if ( FATFS_OK != HwAPI_FatFs_INI_GetKeyInt( "MeasurePlan", "BaseTactLength_s", measurePlanFilename, &cfgMeasurePlan->BaseTactLength_s, MEASURE_PLAN_FILE ) ||
         ...
         FATFS_OK != HwAPI_FatFs_INI_GetKeyInt( "MeasureEnable", "EnableIC1_Comp", measurePlanFilename, &cfgMeasureEnable->enableIC1_Comp, MEASURE_PLAN_FILE ) ||
         FATFS_OK != HwAPI_FatFs_INI_GetKeyInt( "MeasureEnable", "EnableIC2_Comp", measurePlanFilename, &cfgMeasureEnable->enableIC2_Comp, MEASURE_PLAN_FILE ) ||
         FATFS_OK != HwAPI_FatFs_INI_GetKeyInt( "MeasureEnable", "EnableIC3_Comp", measurePlanFilename, &cfgMeasureEnable->enableIC3_Comp, MEASURE_PLAN_FILE ) ||
         FATFS_OK != HwAPI_FatFs_INI_GetKeyInt( "MeasureEnable", "EnableIC4_Comp", measurePlanFilename, &cfgMeasureEnable->enableIC4_Comp, MEASURE_PLAN_FILE ) ||
         ...
         FATFS_OK != HwAPI_FatFs_INI_GetKeyString( "DatafileSettings", "Delimiter", measurePlanFilename, cfgDatafileSettings->delimiter, MEASURE_PLAN_FILE ) )
    {
        ...
        HwAPI_Terminal_SendMessage( "Measure plan file of the plugin "PLUGIN_12_NAME" read - Error.\n" );
        pluginResult.error = 1;
        pluginResult.errorCode = PLG_ERR_READ_MEASURE_PLAN_FILE;
        snprintf( pluginResult.message, sizeof( pluginResult.message ), "Plugin "PLUGIN_12_NAME". Error comes during reading of measure plan file.\n" );
        return pluginResult;
    }
         
    /* Send readed values to terminal */     
    HwAPI_Terminal_SendMessage( "Measure plan file of the plugin "PLUGIN_12_NAME" read - Success.\n" );
...
static void CreateHeaderString( char *string, int32_t stringSize, CfgMeasureEnable_t *cfgMeasureEnable, char *delimiter )
{
    ...
    if ( cfgMeasureEnable->enableIC1_Comp )
    {
        snprintf( string, stringSize, "%sIC1_Comp, V%s", string, delimiter );        
    }
           
    if ( cfgMeasureEnable->enableIC2_Comp )
    {
        snprintf( string, stringSize, "%sIC2_Comp, V%s", string, delimiter );        
    }
           
    if ( cfgMeasureEnable->enableIC3_Comp )
    {
        snprintf( string, stringSize, "%sIC3_Comp, V%s", string, delimiter );        
    }
            
    if ( cfgMeasureEnable->enableIC4_Comp )
    {
        snprintf( string, stringSize, "%sIC4_Comp, V%s", string, delimiter );        
    }
...
static void CreateMeasureString( char *string, int32_t stringSize, MeasureValues_t *measureValues, CfgMeasureEnable_t *cfgMeasureEnable, char *delimiter )
{
    ...
    if ( cfgMeasureEnable->enableIC1_Comp )
    {
        snprintf( string, stringSize, "%s%.3f%s", string, measureValues->valueIC1_Comp, delimiter );        
    }
           
    if ( cfgMeasureEnable->enableIC2_Comp )
    {
        snprintf( string, stringSize, "%s%.3f%s", string, measureValues->valueIC2_Comp, delimiter );        
    }
           
    if ( cfgMeasureEnable->enableIC3_Comp )
    {
        snprintf( string, stringSize, "%s%.3f%s", string, measureValues->valueIC3_Comp, delimiter );        
    }
            
    if ( cfgMeasureEnable->enableIC4_Comp )
    {
        snprintf( string, stringSize, "%s%.3f%s", string, measureValues->valueIC4_Comp, delimiter );        
    }
                                             ...
                                             CfgDatafileSettings_t *cfgDatafileSettings )
{
    ...
    snprintf( measureDataFilename, sizeFilename, "measure/"PLUGIN_12_NAME"/%s_%s.csv", cfgDatafileSettings->prefixDatafileName, pluginsTempString );
                    
    snprintf( pluginsTempString, GetSizeof_pluginsTempString(), "Create '%s' file.\n", measureDataFilename );
    HwAPI_Terminal_SendMessage( pluginsTempString );
                    
    switch ( HwAPI_FatFs_CreateFile( measureDataFilename, 0 ) )
    {
        case FATFS_OK:
        {        
