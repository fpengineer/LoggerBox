/*****************************************************************************************************
    ...
    Overview: Plugin Baranov SilaI1 - V1.0
    ...
    switch ( pluginCommand )
    {
        case CMD_PLUGIN_RUN:
        {
            ...
            *pluginResult = GetConfigData( &cfgMeasurePlan, &cfgMeasureEnable, &cfgDatafileSettings );
            if ( !pluginResult->error )
            {
                ...
                subtactCounter = 0;
                *tactLength_ms = SUBTACT_LENGTH_MS;
            }
            break;
        }
        ...
        case CMD_PLUGIN_TACT:
        {
            ...
            if ( subtactCounter == 0 )
            {
                ...
                HwAPI_Relay_Set( RELAY_EN );

                /* Measure enabled parameters */
                RunMeasure( &measureValues, &cfgMeasureEnable );
                                     ...
                                     CfgDatafileSettings_t *cfgDatafileSettings )
{
    ...
    snprintf( measurePlanFilename, GetSizeof_measurePlanFilename(), "MeasurePlan_"PLUGIN_13_NAME".ini" );
    ...
    if ( FATFS_OK != HwAPI_FatFs_INI_GetKeyInt( "MeasurePlan", "BaseTactLength_s", measurePlanFilename, &cfgMeasurePlan->BaseTactLength_s, MEASURE_PLAN_FILE ) ||
         FATFS_OK != HwAPI_FatFs_INI_GetKeyInt( "MeasurePlan", "TactQ", measurePlanFilename, &cfgMeasurePlan->TactQ, MEASURE_PLAN_FILE ) || 
    ...
    {
        /* Result 'TactQ' in a multiple of 10 */
        cfgMeasurePlan->TactQ = ( cfgMeasurePlan->TactQ / 10 ) * 10;
        
        /* Create error message and return */
        HwAPI_Terminal_SendMessage( "Measure plan file of the plugin "PLUGIN_13_NAME" read - Error.\n" );
        pluginResult.error = 1;
        pluginResult.errorCode = PLG_ERR_READ_MEASURE_PLAN_FILE;
        snprintf( pluginResult.message, sizeof( pluginResult.message ), "Plugin "PLUGIN_13_NAME". Error comes during reading of measure plan file.\n" );
        return pluginResult;
    }
         
    /* Send readed values to terminal */     
    HwAPI_Terminal_SendMessage( "Measure plan file of the plugin "PLUGIN_13_NAME" read - Success.\n" );
